...
void big_int::powModMary( big_int *p, big_int *mod, big_int *mu) {
    red (mod, mu);
    big_int* t = new big_int(this);
    num[0] = 1;
    size = 1;
    sign = 0;
    big_int* precomp [16];
    precomp[0] = new big_int(1);
    for (auto it = std::begin (precomp) + 1; it != std::end (precomp); ++it)
    {
        *it = new big_int (*(it - 1));
        (*it) ->  mul (t);
        (*it) -> red (mod, mu);
    ...
    int half_bytes_size;
    int bits_size = p -> getSizeBits() % 64;
    if (!bits_size) {
        half_bytes_size = 16;
    } else {
        half_bytes_size = bits_size / 4;
    if (bits_size % 4) {
        ++half_bytes_size;
    }
    }

    auto one = new big_int (1);
    auto sixteen = new big_int(16);
    for (int i = half_bytes_size - 1; i > - 1; --i) {
        powMod(sixteen, mod, mu);
        mul (precomp[(p -> num[(p -> size) - 1] >> (4 * i)) & 0xFULL]);
        red(mod, mu);
    }
...
//    std::cout << std::endl;
    ...
    for (int i = p -> size - 2; i > -1; --i){
        for (int j = 15; j > -1; --j){
            powMod(sixteen, mod, mu);
            mul (precomp[(p -> num[i] >> (4 * j)) & 0xFULL]);
//            std::cout << std::hex << ((p -> num[i] >> (4 * j)) & 0xFULL);
            red (mod, mu);
        }
    }
//    std::cout << endl;
